...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/07 11:36:46 by rben-tkh          #+#    #+#             */
/*   Updated: 2023/05/30 20:27:44 by rben-tkh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::setPort(char *port_to_use)
{
	...
	long long	nb = std::atoll(port_to_use);
...
void	Server::setPass(char *pass_to_use)
{
	...
	for (i = 0; pass_to_use[i] && !std::islower(pass_to_use[i]); i++);
	for (j = 0; pass_to_use[j] && !std::isupper(pass_to_use[j]); j++);
	for (k = 0; pass_to_use[k] && !std::isdigit(pass_to_use[k]); k++);
	if (pass_len < 8 || i == pass_len || j == pass_len || k == pass_len)
	{
		std::string	message(RED "Password must contain at least " UNDER);
		if (pass_len < 8)
...
void	Server::init_sock(void)
{
	...
	if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt)) == ERROR)
...
void	Server::init_client(int i, int cfd)
{
	...
	message = " " + users[cfd].nick + " :You made it ! " + users[cfd].nick + "!" + users[cfd].user + "@localhost";
	Command::send_code("001", message, cfd);
	std::string codes[5] = {"002", "003", "004", "005", "375"};
	for (int j(0); j < 5; j++)
		Command::send_code(codes[j], EMPTY, cfd);
	Command::send_code("372", " : Welcome to Rayan's and Lais's homemade IRC server !", cfd);
