...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/14 06:10:25 by rben-tkh          #+#    #+#             */
/*   Updated: 2023/05/30 20:46:31 by rben-tkh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Command::send_code(std::string code, std::string message, int cfd)
{
	message = ":localhost " + code + message + "\r\n";
...
{
	std::string	message = ":localhost PONG " + cmd[1] + "\r\n";
	if (send(cfd, message.c_str(), message.length(), 0) == ERROR)
		std::cerr << RED "Client " << cfd << SEND_ERROR << RESET;
}
...
void	Command::oper(std::string user, std::string password, int cfd)
{
	if (users[cfd].g_oper == false)
	{
		if (user != ADMIN_USER)
		{
			send_code("491", users[cfd].nick + " : Wrong username", cfd);
			std::cout << YELLOW "Client " << cfd << " : Tried to become an operator\n";
			return ;
		}
		if (password != ADMIN_PASS)
		{
			send_code("464", users[cfd].nick + " : Wrong password", cfd);
			std::cout << YELLOW "Client " << cfd << " : Tried to become an operator\n";
			return ;
		}
		users[cfd].g_oper = true;
		send_code("381", users[cfd].nick + " : You are now an oper", cfd);
...
void	Command::nick(std::vector<std::string> cmd, int cfd)
{
	for (std::map<int, t_user>::iterator it = users.begin(); it != users.end(); ++it)
	{
		if (it->second.nick == cmd[1])
		{
			...
			send_code("433", message, cfd);
			return ;
		}
	}
	std::string	message = ":" + users[cfd].nick + "!" + users[cfd].user + "@localhost NICK " + cmd[1] + "\r\n";
	if (send(cfd, message.c_str(), message.length(), 0) == ERROR)
		std::cerr << RED "Client " << cfd << SEND_ERROR << RESET;
...
void	Command::topic(std::vector<std::string> cmd, int cfd)
{
	...
	if (it != channels.end())
	{
		...
		{
			send_code("442", " " + cmd[1] + " :You're not on that channel", cfd);
			return ;
		}
		if (cmd.size() < 3)
		{
			...
			{
				send_code("331", " " + cmd[1] + " :No topic is set", cfd);
				return ;
			}
			send_code("332", " " + users[cfd].nick + " " + cmd[1] + " " + channels[cmd[1]].chan_topic, cfd);
			//send_code("333", " " + users[cfd].nick + " " + cmd[1], cfd);
			return ;
		}
		else if (!oper_inchan(cmd[1], cfd) && channels[cmd[1]].mode_topic)
		{
			send_code("482", " " + cmd[1] + " :You're not channel operator", cfd);
			return ;
		}
		...
		for (size_t i = 0; i < channels[cmd[1]].chan_users.size(); i++)
		{
			...
			msg += "@localhost TOPIC " + cmd[1] + " " + channels[cmd[1]].chan_topic + "\r\n";
		...
		send_code("403", " " + cmd[1] + " :No such channel", cfd);
}
...
void	Command::kick(std::vector<std::string> cmd, int cfd)
{
	...
	for (size_t j = 0; j < channels[cmd[1]].chan_users.size(); j++)
	{
		...
		{
			std::string msg = ":" + users[cfd].nick + "!" + users[cfd].user + "@localhost KICK " + cmd[1] + " " + cmd[2] + "\r\n";
			for (size_t i = 0; i < channels[cmd[1]].chan_users.size(); i++)
			{
				if (send(channels[cmd[1]].chan_users[i]->cfd, msg.c_str(), msg.length(), 0) == ERROR)
					std::cerr << RED "Client " << channels[cmd[1]].chan_users[i]->cfd << SEND_ERROR << RESET;
...
void	Command::invite(std::vector<std::string> cmd, int cfd)
{
	...
	if (it == channels.end())
	{
		send_code("403", " " + cmd[2] + " :No such channel", cfd);
		return ;
	}
	for (size_t j = 0; j < channels[cmd[2]].chan_users.size(); j++)
	{
		...
		{
			send_code("443", cmd[1] + " " + cmd[2] + " :is already on channel", cfd);
			return ;
		}
	}
	if (user_inchan(cmd[2], cfd) == -1)
	{
		send_code("442", " " + cmd[2] + " :User not on channel", cfd);
		return ;
	}
	if (!oper_inchan(cmd[2], cfd) && channels[cmd[2]].mode_inv)
	{
		send_code("482", " " + cmd[2] + " :You're not channel operator", cfd);
		return ;
	}
	for (size_t j = 0; j < clients.size(); j++)
	{
		...
		{
			send_code("341", " " + cmd[2] + " " + cmd[1] + " " + cmd[2], cfd);
			std::string msg = ":" + users[cfd].nick + "!" + users[cfd].user + "@localhost INVITE " + cmd[1] + " " + cmd[2] + "\r\n";
...
void	Command::mode(std::vector<std::string> cmd, int cfd)
{
	...
	if (user_inchan(cmd[1], cfd) == -1)
	{
		send_code("442", " " + cmd[1] + " :User not on channel", cfd);
		return ;
	}
	if (!oper_inchan(cmd[1], cfd))
	{
	...
	else if (cmd[2] == "+k")
	{
		...
		return ;
	std::string msg = ":" + users[cfd].nick + "!" + users[cfd].user + "@localhost MODE ";
	msg += cmd[1] + " " + cmd[2];
	if (cmd.size() == 4)
		msg += " " + cmd[3];
	msg += "\r\n";
...
void	Command::join(std::vector<std::string> cmd, int cfd)
{
	...
	for (size_t i = 0; i < chan.size(); i++)
	{
		...
		topic(top, cfd);
		msg = ":localhost 353 " + users[cfd].nick + " = " + chan[i] + " ";
		for (size_t j = 0; j < channels[chan[i]].chan_users.size(); j++)
		...
		for (size_t j = 0; j < channels[chan[i]].chan_users.size(); j++)
		{
			if (send(channels[chan[i]].chan_users[j]->cfd, msg.c_str(), msg.length(), 0) == ERROR)
				std::cerr << RED "Client " << channels[chan[i]].chan_users[j]->cfd << SEND_ERROR << RESET;
			send_code("366", " " + chan[i], channels[chan[i]].chan_users[j]->cfd);
		}
